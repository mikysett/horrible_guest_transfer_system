...
function stampa_commenti(&$commenti, &$num_com, $tipo_com) {
	...
	for($i = 0 ; $i < $num_com ; $i++) {
		...
		echo '<span class="testo_com">'.formatta_visualizzazione($commenti[$i]['testo']);
		if($commenti[$i]['operatore'] != '') echo '<span class="op_com">-'.formatta_visualizzazione($commenti[$i]['operatore']).'</span>';
