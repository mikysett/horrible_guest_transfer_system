...
if(isset($_SESSION['tipo_user']) && $_SESSION['tipo_user'] == 0) { // Si verifica che sia l'amministratore
	...
	if(isset($_GET['mp'])) {
		...
		$info_pre['nome'] 					 = str_show($pre_spe[0]['nome']);
		...
		$info_pre['num_tel'] 				 = str_show($pre_spe[0]['num_tel']);
		$info_pre['num_tel_sec'] 			 = str_show($pre_spe[0]['num_tel_sec']);
		$info_pre['email'] 					 = str_show($pre_spe[0]['email']);
		$info_pre['email_sec'] 				 = str_show($pre_spe[0]['email_sec']);
		
		$info_pre['data_arr'] 				 = &$pre_spe[0]['data_arr'];
		$info_pre['ora_arr'] 				 = &$pre_spe[0]['ora_arr'];
		$info_pre['luogo_arr'] 				 = str_show($pre_spe[0]['luogo_arr']);
		$info_pre['volo_arr']				 = str_show($pre_spe[0]['volo_arr']);
		$info_pre['ora_taxi_arr']			 = &$pre_spe[0]['ora_taxi_arr'];
		$info_pre['taxi_arr']				 = str_show($pre_spe[0]['taxi_arr']);
		$info_pre['porto_partenza_arr']	 = str_show($pre_spe[0]['porto_partenza_arr']);
		$info_pre['barca_arr']				 = str_show($pre_spe[0]['barca_arr']);
		$info_pre['porto_arrivo_arr']		 = str_show($pre_spe[0]['porto_arrivo_arr']);
		$info_pre['ora_barca_arr']			 = &$pre_spe[0]['ora_barca_arr'];
		$info_pre['stato_arr']				 = &$pre_spe[0]['stato_arr'];
		$info_pre['operatore_arr']			 = str_show($pre_spe[0]['operatore_arr']);
		...
		$info_pre['luogo_par'] 				 = str_show($pre_spe[0]['luogo_par']);
		$info_pre['volo_par']				 = str_show($pre_spe[0]['volo_par']);
		$info_pre['taxi_par']				 = str_show($pre_spe[0]['taxi_par']);
		$info_pre['ora_taxi_par']			 = &$pre_spe[0]['ora_taxi_par'];
		$info_pre['porto_partenza_par']	 = str_show($pre_spe[0]['porto_partenza_par']);
		$info_pre['barca_par']				 = str_show($pre_spe[0]['barca_par']);
		$info_pre['porto_arrivo_par']		 = str_show($pre_spe[0]['porto_arrivo_par']);
		$info_pre['ora_barca_par']			 = &$pre_spe[0]['ora_barca_par'];
		$info_pre['stato_par']				 = &$pre_spe[0]['stato_par'];
		$info_pre['operatore_par']			 = str_show($pre_spe[0]['operatore_par']);
	...
	elseif(isset($_POST['nome'])) {
		...
		$info_pre['nome'] 					 = str_save_sensitive($_POST['nome']);
		$info_pre['pax_ad'] 					 = &$_POST['pax_ad'];
		$info_pre['pax_bam'] 				 = &$_POST['pax_bam'];
		$info_pre['camera'] 					 = str_save($_POST['camera']);
		...
		$info_pre['num_tel'] 				 = str_save_nobreak($_POST['num_tel']);
		$info_pre['num_tel_sec'] 			 = str_save_nobreak($_POST['num_tel_sec']);
		$info_pre['email'] 					 = str_save_nobreak($_POST['email']);
		$info_pre['email_sec'] 				 = str_save_nobreak($_POST['email_sec']);
		...
		$info_pre['luogo_arr'] 				 = str_save($_POST['luogo_arr']);
		$info_pre['volo_arr']				 = str_save($_POST['volo_arr']);
		...
		$info_pre['operatore_arr']			 = str_save($_POST['operatore_arr']);
		...
		$info_pre['luogo_par'] 				 = str_save($_POST['luogo_par']);
		$info_pre['volo_par']				 = str_save($_POST['volo_par']);
		...
		$info_pre['operatore_par']			 = str_save($_POST['operatore_par']);
		...
		// Se la prenotazione va creata
			...
			try {
				...
				echo 'Create Reservation Exception -> ';
			...
			try {
				...
				echo 'Take id of new reservation Exception -> ';
			 	var_dump($e->getMessage());
			}
			...
			if(isset($_POST['testo_com'.$i])) {
				...
				for($j = 0 ; $j < $num_com_rapidi ; $j++) {
					if(isset($_POST[$i.'rapid_com'.$j])) {
						...
						try {
						 	$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
							$db->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
						$db->query('INSERT INTO commenti (id_transfer,tipo_commento,testo,data_creazione,reception) VALUES ('.$info_pre['id'].','.$i.',"'.str_save($_POST[$i.'rapid_com'.$j]).'",'.$data_adesso.',1)');
						}
						catch(Exception $e) {
							echo 'Add comments Exception -> ';
						 	var_dump($e->getMessage());
						}
						$db->connection = NULL;
				...
				if($_POST['testo_com'.$i] != '') {
					...
					try {
					 	$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
						$db->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
					$db->query('INSERT INTO commenti (id_transfer,tipo_commento,testo,operatore,data_creazione'.$reparti.') VALUES ('.$info_pre['id'].','.$i.',"'.str_save_sensitive($_POST['testo_com'.$i]).'","'.str_save($_POST['operatore_com'.$i]).'",'.$data_adesso.$val_reparti.')');
					}
					catch(Exception $e) {
						echo 'Add comment Exception -> ';
					 	var_dump($e->getMessage());
					}
					$db->connection = NULL;
...
<head><?php
	echo '<title>';
	if(isset($_GET['mp']) || isset($_POST['nome']))  echo str_show($info_pre['nome']) . ' - MODIFICA TRANSFER';
		...
		?><table>
				<tr>
					...
					<td colspan="2" style="min-width: 230px">
						<input placeholder="NOME TRANSFER" type="text" name="nome" class="field" value="<?php echo str_show($info_pre['nome']) ?>" autofocus />
					</td>
					<td colspan="1">
				...
				<tr>
					<td colspan="3">
						<input placeholder="EMAIL" type="text" name="email" class="field" value="<?php echo str_show($info_pre['email']) ?>" />
					</td>
					<td colspan="3">
						<input placeholder="TELEFONO" type="text" name="num_tel" class="field" value="<?php echo str_show($info_pre['num_tel']) ?>" />
					</td>
				</tr>
				<tr>
					<td colspan="3">
						<input placeholder="EMAILS SECONDARIE" type="text" name="email_sec" class="field" value="<?php echo str_show($info_pre['email_sec']) ?>" />
					</td>
					<td colspan="2">
						<input placeholder="TELEFONI SECONDARI" type="text" name="num_tel_sec" class="field" value="<?php echo str_show($info_pre['num_tel_sec']) ?>" />
					</td><?php
					echo '<td colspan="1">';
			...
			if($modifica_pre == TRUE) {
				...
				if($num_note_gg > 0) {
					...
					for($x = 0 ; $x < $num_note_gg ; $x++) {
						...
						echo '<span class="testo_note">'.str_show($note_gg[$x]['testo']).'</span>';
