...
if(isset($_SESSION['tipo_user']) && $_SESSION['tipo_user'] == 0) { // Si verifica che sia l'amministratore
	...
	if(isset($_POST['mezzo'])) {
		...
		if($_POST['luogo_arrivo'] != '')  $luogo_arrivo = str_save($_POST['luogo_arrivo']);	
	}
...
<body><?php
		...
		echo ' LUOGO ARR/PAR ';
		echo '<input type="text" name="luogo_arrivo" class="field_stat" value="'.str_show($luogo_arrivo).'" />';
		...
		if($mezzo != '') echo ' PER '.str_show($mezzo);
